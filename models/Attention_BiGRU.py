...
class Net(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, batch_size, num_layers=1,num_words=50000):
        ...
        self.attention = nn.MultiheadAttention(300, 1)
...
if __name__ == '__main__':
    ...
    # 装载之前训练的部分继续训练
    # net.load_state_dict(torch.load(PathRoot+'log/'+netName+'/90.pkl'))
    ...
    print('-------加载数据---------')
    train_db = getdata(filename=PathRoot + 'data/' + dataName + '/train.csv', num_words=num_words,
    ...
    print('-------模型测试中---------')
    test_db = getdata(filename=PathRoot + 'data/' + dataName + '/test.csv', num_words=num_words,
                      max_tokens=sequence_len)
