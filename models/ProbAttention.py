...
if __name__ == '__main__':
    PathRoot = '../'
    netName = 'GRU_ProbHeadAttention'
    writer = SummaryWriter(log_dir=PathRoot +'log/'+ netName,comment="GRU_MulHeadAttention,lr=0.01，batch_size=64,layer=1,two_head,embedding=200000,head=1")
    ...
    print('-------加载数据---------')
    train_db = getdata(filename=PathRoot + 'data/train.csv', num_words=num_words, max_tokens=sequence_len)
    val_db = getdata(filename=PathRoot + 'data/val.csv', num_words=num_words, max_tokens=sequence_len)
    print('-------训练开始---------')
    ...
    for epoch in range(N_EPOCHS):
        ...
        if ((epoch + 1) % 5 == 0) & ((epoch + 1) > 80):
            torch.save(net.state_dict(), '../log/' + netName + '/' + str(epoch + 1) + '.pkl')
    writer.close()

    print('-------模型测试中---------')

    test_db = getdata(filename=PathRoot + 'data/testData_10w.csv', num_words=num_words, max_tokens=sequence_len)
    ...
    with open(PathRoot + 'log/' + netName + '/测试结果.txt', 'a') as file_handle:
        file_handle.write('TP:' + str(TP) + ' TN:' + str(TN) + ' FP:' + str(FP) + ' FN:' + str(FN))
        file_handle.write(
